/**
 * Aplica 20260909220000_gladys_rodrigo_pago_maio.sql:
 *   Gladys Rodrigo Gutierrez (Liberty - GC) — os R$ 6.000,00 pagos em maio
 *   deixam de ser "entrada da venda" e viram a parcela 1 paga em 15/05/2026,
 *   para constar no card Pago de maio.
 *
...
const MIGRACAO = 'supabase/migrations/20260909220000_gladys_rodrigo_pago_maio.sql';
...
const FICHA_SQL = `
  SELECT s.name, s.sale_value, s.down_payment, s.total_installments, s.paid_installments, s.status,
         (SELECT string_agg(format('#%s %s@%s%s', i->>'number', i->>'value', i->>'dueDate',
...
try {
  ...
  const { rows: [antes] } = await client.query(FICHA_SQL);
  console.log('ANTES :', antes);

  await client.query(sql);

  const { rows: [depois] } = await client.query(FICHA_SQL);
