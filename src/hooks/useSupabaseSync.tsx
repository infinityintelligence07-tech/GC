...
async function fetchAll(activeCompanyId?: string | null) {
  ...
  const acs: AC[] = (acsRes.data ?? []).map((r: any) => ({
    ...
    metaTaxaEmDiaBaseMes: r.meta_taxa_em_dia_base_mes ?? undefined,
  ...
  const rules: FinancialRules | null = rulesRes.data
    ? {
        ...
        metaTaxaEmDiaBaseMes: (rulesRes.data as any).meta_taxa_em_dia_base_mes ?? undefined,
        ...
        emDiaNovosMeta: (rulesRes.data as any).em_dia_novos_meta != null ? Number((rulesRes.data as any).em_dia_novos_meta) : undefined,
