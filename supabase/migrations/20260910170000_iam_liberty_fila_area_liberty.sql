-- Pull do IAM Control: contrato de produto Liberty entra na empresa Liberty - GC
-- (10/09/2026).
--
-- Desde 28/08 toda ficha vinda do IAM Control caía na empresa IAM - GC e a fila
-- "IAM Control → GC" separava Liberty x IAM só por filtro visual. Regra nova do
-- financeiro: contrato Liberty / Liberty Begin / BEGIN nasce (e fica pendente)
-- na fila IAM Control → GC da ÁREA LIBERTY; ao ser conciliado entra direto na
-- carteira/dashboard da Liberty. Demais treinamentos seguem na IAM - GC.
--
-- Única mudança na função: v_company_id passa a ser
-- public.resolve_gc_company_id(v_produto) (Liberty x IAM pelo produto), em vez
-- de sempre a empresa IAM. O restante é idêntico a 20260909210000.
--
-- No fim, move para a Liberty - GC os contratos Liberty já importados na IAM - GC
-- (com seus itens de conciliação etc.), exceto fichas com status IAM "NOVO",
-- que o pull não importa mais e ficam para decisão manual.
...
-- ─── Dados: contratos Liberty já importados na IAM - GC vão para a Liberty - GC ──
-- Leva junto itens de conciliação (fila IAM Control → GC, histórico), casos de
-- cancelamento, comissões, documentos ZapSign e erros de importação.
-- Fica de fora ficha com status IAM "NOVO": o pull não importa NOVO desde
-- 24/08, essas fichas são resíduo anterior e não entram em fila nenhuma.
DO $$
DECLARE
  v_iam uuid := (SELECT id FROM public.companies WHERE slug = 'iam' LIMIT 1);
  v_lib uuid := (SELECT id FROM public.companies WHERE slug = 'liberty' LIMIT 1);
  n_alunos int; n_itens int; n_casos int; n_com int; n_zap int; n_err int;
BEGIN
  IF v_iam IS NULL OR v_lib IS NULL THEN
    RAISE EXCEPTION 'empresas iam/liberty não encontradas';
  END IF;

  CREATE TEMP TABLE mv_liberty AS
    SELECT s.id
    FROM public.students s
    WHERE s.company_id = v_iam
      AND s.iam_control_aluno_id IS NOT NULL
      AND public.product_is_liberty_gc(s.product)
      AND upper(coalesce(s.iam_control_contrato_status, '')) <> 'NOVO';

  UPDATE public.students s
  SET company_id = v_lib,
      updated_at = now(),
      history = coalesce(s.history, '[]'::jsonb) || jsonb_build_array(jsonb_build_object(
        'date', to_jsonb(now()),
        'type', 'Sistema',
        'text', 'Contrato IAM Control de produto Liberty movido da empresa IAM - GC para a Liberty - GC (fila IAM Control → GC da área Liberty).'
      ))
  WHERE s.id IN (SELECT id FROM mv_liberty);
  GET DIAGNOSTICS n_alunos = ROW_COUNT;

  UPDATE public.conciliacao_items SET company_id = v_lib, updated_at = now()
  WHERE student_id IN (SELECT id FROM mv_liberty) AND company_id = v_iam;
  GET DIAGNOSTICS n_itens = ROW_COUNT;

  UPDATE public.cancellation_cases SET company_id = v_lib
  WHERE student_id IN (SELECT id FROM mv_liberty) AND company_id = v_iam;
  GET DIAGNOSTICS n_casos = ROW_COUNT;

  UPDATE public.commissions SET company_id = v_lib
  WHERE student_id IN (SELECT id FROM mv_liberty) AND company_id = v_iam;
  GET DIAGNOSTICS n_com = ROW_COUNT;

  UPDATE public.zapsign_documents SET company_id = v_lib
  WHERE student_id IN (SELECT id FROM mv_liberty) AND company_id = v_iam;
  GET DIAGNOSTICS n_zap = ROW_COUNT;

  UPDATE public.conciliacao_import_errors SET company_id = v_lib
  WHERE student_id IN (SELECT id FROM mv_liberty) AND company_id = v_iam;
  GET DIAGNOSTICS n_err = ROW_COUNT;

  RAISE NOTICE 'Liberty ← IAM: % alunos, % itens conciliação, % casos, % comissões, % zapsign, % erros import',
    n_alunos, n_itens, n_casos, n_com, n_zap, n_err;

  DROP TABLE mv_liberty;
END $$;
