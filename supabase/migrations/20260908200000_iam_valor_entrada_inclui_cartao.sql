-- IAM passou a incluir o cartão de crédito em `valor_entrada` → entrada dobrava.
--
-- Casos reais (08/09/2026):
--   * Sidneia / Trainer: cartão R$ 5.000 + boleto R$ 1.500. O IAM manda
--     valor_entrada = 5000 (já com o cartão). O GC fazia v_down := 5000
--     (override) e depois somava v_cartao (5000) → entrada 10.000 > contrato,
--     caía no ramo "quitado": entrada 6.500 e nenhum boleto.
--   * Lidiane / Missão Governar: cartão R$ 4.000 + cartão R$ 11.707,16
--     (pendência, link). valor_entrada = 4000 → entrada 8.000 em vez de 4.000.
--
-- A migração 20260903130000 assumia que valor_entrada NÃO incluía o cartão
-- (era 0 quando só havia cartão). O export atual do IAM inclui. Regra nova:
-- se valor_entrada já cobre (entrada por PIX/débito/espécie + cartão), o
-- cartão não é somado de novo; se valor_entrada é menor (formato antigo, só a
-- entrada sem cartão), mantém o comportamento anterior e soma o cartão.
-- O restante (corte do detalhe de parcelas do cartão, pendência, etc.) segue
-- igual.
...
BEGIN
  ...
  IF nullif(p_treinamento->>'valor_entrada', '') IS NOT NULL THEN
    ...
    -- Export atual do IAM: valor_entrada já inclui o cartão cobrado. Se cobre
    -- o que o loop apurou (entrada + cartão), não soma o cartão de novo.
    v_cartao_na_entrada := v_cartao > 0.0049 AND v_entrada_iam >= v_down + v_cartao - 0.05;
  ...
  IF v_cartao > 0.0049 THEN
    IF NOT v_cartao_na_entrada THEN
...
COMMENT ON FUNCTION public.iam_treinamento_financeiro(jsonb) IS
  'Financeiro IAM→GC. Cartão de crédito sem marca de pendência é recebido à vista pela empresa; se valor_entrada do IAM já inclui o cartão, ele não é somado de novo. Forma marcada como pendência fica em aberto até o contrato conciliar.';
