-- Roteamento IAM → empresa correta: Liberty/Liberty Begin → Liberty - GC; demais → IAM - GC.

CREATE OR REPLACE FUNCTION public.product_is_liberty_gc(p_product text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    ...
    ELSE (
      ...
      SELECT
        ...
        OR p LIKE '% liberty'
        OR p LIKE '% liberty %'
        OR p LIKE '%liberty begin%'
      FROM norm
    )
  END;
$$;

COMMENT ON FUNCTION public.product_is_liberty_gc(text) IS
  'True para produtos da carteira Liberty - GC (Liberty, Liberty Begin, BEGIN, etc.).';

CREATE OR REPLACE FUNCTION public.resolve_gc_company_id(p_product text)
RETURNS uuid
LANGUAGE sql
STABLE
AS $$
  SELECT CASE
    WHEN public.product_is_liberty_gc(p_product) THEN (
      SELECT id FROM public.companies WHERE slug = 'liberty' LIMIT 1
    )
    ELSE (
      SELECT id FROM public.companies WHERE slug = 'iam' LIMIT 1
    )
  END;
$$;

COMMENT ON FUNCTION public.resolve_gc_company_id(text) IS
  'Empresa GC destino pelo produto/treinamento (Liberty vs IAM).';
...
BEGIN
  ...
  -- Sync IAM: empresa destino sempre pelo produto (Liberty vs IAM).
...
-- Corrige fichas Liberty que caíram na empresa IAM - GC.
UPDATE public.students s
SET company_id = public.resolve_gc_company_id(s.product),
    updated_at = now()
WHERE s.company_id = public.resolve_gc_company_id('Confronto')
  AND public.product_is_liberty_gc(s.product)
  AND NOT EXISTS (
    SELECT 1
    FROM public.students s2
    WHERE s2.company_id = public.resolve_gc_company_id(s.product)
      AND s2.id <> s.id
      AND s2.cpf_digits <> ''
      AND s2.cpf_digits = s.cpf_digits
      AND lower(btrim(coalesce(s2.product, ''))) = lower(btrim(coalesce(s.product, '')))
      AND lower(btrim(coalesce(s2.ciclo, ''))) = lower(btrim(coalesce(s.ciclo, '')))
  );
