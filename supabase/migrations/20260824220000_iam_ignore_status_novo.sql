-- IAM Control: contratos com status NOVO não entram no GC.
-- Remove fichas criadas só pelo IAM com status NOVO; limpa tag nos demais.

-- 1) Remove fichas IAM NOVO sem vínculo Kamino nem caso de cancelamento
DELETE FROM public.conciliacao_import_errors e
USING public.students s
WHERE e.student_id = s.id
  AND s.iam_control_contrato_status = 'NOVO'
  AND NOT EXISTS (
    SELECT 1 FROM public._kamino_sync_staging k
    WHERE k.skey = public.gc_student_key(s.name, s.product)
  )
  AND NOT EXISTS (
    SELECT 1 FROM public.cancellation_cases cc WHERE cc.student_id = s.id
  );
...
WHERE s.iam_control_contrato_status = 'NOVO'
  AND NOT EXISTS (
    SELECT 1 FROM public._kamino_sync_staging k
    WHERE k.skey = public.gc_student_key(s.name, s.product)
  )
...
-- 2) Fichas Kamino/cancelamento: remove apenas a marca IAM NOVO (mantém financeiro Kamino)
UPDATE public.students s
SET
  iam_control_contrato_status = NULL,
  iam_control_contrato_id     = NULL,
  updated_at                  = now()
WHERE s.iam_control_contrato_status = 'NOVO';
...
BEGIN
  ...
  IF v_student_id IS NOT NULL THEN
    ...
    UPDATE public.students s SET
      ...
      sale_value                  = CASE WHEN v_sem_parcelas THEN v_fin.sale_value ELSE s.sale_value END,
      down_payment                = CASE WHEN v_sem_parcelas THEN v_fin.down_payment ELSE s.down_payment END,
      total_installments          = CASE WHEN v_sem_parcelas THEN v_fin.total_installments ELSE s.total_installments END,
      installment_value           = CASE WHEN v_sem_parcelas THEN v_fin.installment_value ELSE s.installment_value END,
      installments                = CASE WHEN v_sem_parcelas THEN v_fin.installments ELSE s.installments END,
      paid_installments           = CASE WHEN v_sem_parcelas THEN v_fin.paid_installments ELSE s.paid_installments END
    WHERE s.id = v_student_id;
...
COMMENT ON FUNCTION public.iam_control_upsert_one_contract(jsonb, text, jsonb, text) IS
  'Upsert IAM→GC por treinamento. Ignora status NOVO. Em UPDATE, preserva financeiro Kamino quando a ficha já tem parcelas.';
