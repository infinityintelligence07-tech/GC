-- valor_entrada do IAM cobrindo SÓ PARTE do cartão → entrada somava o cartão
-- de novo e o saldo virava parcelas fantasmas.
--
-- Caso real (12/09/2026) — Rafael Hilário / Liberty Begin R$ 23.400:
--   formas: cartão R$ 5.875 (entrada) + cartão R$ 2.364 (10x) + link
--   R$ 15.161 (pendência). IAM manda valor_entrada = 5875 (só o cartão da
--   entrada), parcelas = 2, valor_parcela = 5875, parcelas_detalhe = [5875, 2364].
--   O GC fazia: v_down := 5875 (override) → 5875 < 0 + 8239 → "cartão não está
--   na entrada" → v_down := 5875 + 8239 = 14.114 (o 5.875 contado duas vezes).
--   Depois descartava o detalhe (só cartão) e caía em `parcelas = 2` do IAM →
--   2 × 5.875 a vencer. Resultado: 14.114 + 11.750 = 25.864 num contrato de
--   23.400, e a pendência (link 15.161) sumia.
--
-- Regra nova (validada contra 424 treinamentos reais do IAM em 12/09/2026):
--   1. valor_entrada do IAM inclui o cartão À VISTA (forma sem `parcelas`) e
--      NÃO inclui o cartão PARCELADO (`parcelas` > 1, "à prazo"). Quando o
--      cartão à vista já cabe em valor_entrada, soma-se só o parcelado; senão
--      (formato antigo, valor_entrada sem cartão) soma-se o cartão todo.
--      A comparação ignora boleto 1x, que entra em v_down por conveniência mas
--      não é entrada para o IAM (Caio: cartão 6.000 + boleto 15.411 1x dava
--      entrada 12.000).
--   2. Cartão integral já apurado: se recebido + pendência em aberto fecha o
--      contrato, não há nada a parcelar (parcelas := 0) — vale também para
--      contrato pendente. A pendência vira a parcela nº 1 com tag
--      entrada-pendente, como antes (Angelita: cartão 6.000 + link 5.646
--      gerava uma 2ª parcela fantasma de 6.000).
-- Rafael → entrada 8.239 (cartões, pagos) + 1 parcela de 15.161 (link).
