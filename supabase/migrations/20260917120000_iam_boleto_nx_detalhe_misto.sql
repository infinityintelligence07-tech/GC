-- Boleto parcelado (NX) listado como UM item no detalhe — inclusive quando o
-- detalhe ainda traz fragmentos do cartão/entrada (detalhe misto).
--
-- Caso real (17/09/2026) — Lorena / Missão Governar R$ 20.011:
--   formas: cartão R$ 2.000 + boleto R$ 18.011 em 18x.
--   parcelas_detalhe = [1000, 18011], parcelas top-level = 2.
--   O strip do cartão falha (1000 ≠ 2000); o collapse posterior vira
--   1 parcela de R$ 18.011 em vez de 18 × R$ 1.000,61.
--
-- Regra: se alguma forma parcelada (boleto etc., não cartão) tem `parcelas` > 1
-- e o valor bate com QUALQUER item do detalhe, o detalhe vira N parcelas iguais
-- desse valor (centavos na última). Também cobre detalhe vazio + forma Nx.
-- Layane (detalhe residual [11325]) continua coberta.
...
BEGIN
  ...
  -- um lump no detalhe — sozinho ou misturado com fragmentos da entrada/cartão.
  -- Abre em N parcelas iguais e descarta o restante do detalhe (já coberto pela
  -- entrada/cartão em v_down).
  FOR v_i IN 0..greatest(jsonb_array_length(v_formas) - 1, -1) LOOP
    v_fp := v_formas->v_i;
    v_forma := v_fp->>'forma';
    v_valor := coalesce(nullif(v_fp->>'valor', '')::numeric, 0);
