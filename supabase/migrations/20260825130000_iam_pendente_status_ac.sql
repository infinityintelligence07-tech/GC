-- IAM PENDENTE → status operacional "Pendente" (Manual) + AC pela esteira se vazio.
...
DECLARE
  ...
  v_ciclo text := lower(btrim(coalesce(p_ciclo, '')));
  v_prod text;
  v_existing_ac text;
  v_last uuid;
  v_ids uuid[];
...
BEGIN
  ...
  IF v_student_id IS NOT NULL THEN
    ...
    UPDATE public.students s SET
      ...
      status                      = CASE
                                      WHEN v_is_pendente THEN 'Pendente'
                                      ELSE s.status
                                    END,
      status_mode                 = CASE
                                      WHEN v_is_pendente THEN 'Manual'
                                      ELSE s.status_mode
                                    END,
...
COMMENT ON FUNCTION public.iam_control_upsert_one_contract(jsonb, text, jsonb, text) IS
  'Upsert IAM→GC. PENDENTE → status Pendente/Manual + AC pela esteira se vazio. Ignora NOVO. Preserva Kamino.';

-- Reparo: fichas já no GC com contrato IAM PENDENTE
DO $$
DECLARE
  r record;
  v_ac text;
BEGIN
  FOR r IN
    SELECT s.id, s.company_id, s.cpf, s.product, s.ciclo, nullif(btrim(coalesce(s.ac, '')), '') AS ac
    FROM public.students s
    WHERE upper(coalesce(s.iam_control_contrato_status, '')) = 'PENDENTE'
  LOOP
    v_ac := r.ac;
    IF v_ac IS NULL THEN
      v_ac := public.next_ac_from_esteira(r.company_id, r.cpf, r.product, r.ciclo, r.id);
    END IF;
