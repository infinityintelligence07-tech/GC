-- Fila IAM CONTROL -> GC: contrato ainda NAO aprovado no GC continua com o
-- financeiro antigo (ex.: cartao de credito 3x/12x virando parcelas em aberto)
-- mesmo depois da correcao em iam_treinamento_financeiro (20260903130000).
--
-- Causa: v_preserva_fin travava a reescrita sempre que a ficha ja tinha
-- parcelas e o status nao era PENDENTE_*. PARA_CONCILIAR e CONCILIADO ainda
-- sem aprovacao GC caiam nessa regra e nunca eram recalculados.
--
-- Regra nova: enquanto o GC nao aprovou (iam_gc_conciliado_at IS NULL), a
-- estrutura financeira vem do IAM a cada pull (baixas ja registradas no GC
-- continuam preservadas pelo merge por numero da parcela). Depois da
-- aprovacao, preserva como antes (so reabertura legitima reescreve).
...
COMMENT ON FUNCTION public.iam_control_upsert_one_contract(jsonb, text, jsonb, text) IS
  'Upsert IAM→GC. Antes da aprovação GC o financeiro vem do IAM a cada pull (baixas GC preservadas). Aprovação GC (iam_gc_conciliado_at) só reabre quando o IAM muda de status não pendente para pendente.';
