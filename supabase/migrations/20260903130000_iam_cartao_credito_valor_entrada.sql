-- Cartão de crédito parcelado vinha como N parcelas pendentes no GC.
--
-- Causa: a migração 20260828033000 somava o cartão de crédito em v_down, mas
-- logo abaixo `valor_entrada` (que o IAM sempre envia, normalmente 0 quando
-- não há PIX/débito/espécie) sobrescrevia v_down, apagando o cartão. Como o
-- payload também traz `parcelas_detalhe` com as N parcelas do cartão, o GC
-- montava N recebíveis em aberto — mas a operadora repassa o valor à vista:
-- não importa em quantas vezes o cliente parcelou no cartão dele, para a
-- empresa entra uma vez só.
--
-- Correção: o cartão de crédito (contrato não pendente) é acumulado em
-- separado (v_cartao) e somado à entrada DEPOIS do override de valor_entrada.
-- Quando o detalhe de parcelas é só o cartão, o detalhe é descartado — o
-- contrato cai no ramo "quitado" (sem recebíveis futuros, direto no card PAGO).
...
BEGIN
  FOR v_i IN 0..greatest(jsonb_array_length(v_formas) - 1, -1) LOOP
    ...
    ELSIF NOT v_pendente AND public.iam_forma_is_cartao_credito(v_forma) THEN
      ...
      -- das parcelas do cliente. Acumulado à parte para sobreviver ao
      -- override de valor_entrada abaixo.
  ...
  IF NOT v_pendente AND v_cartao > 0.0049 THEN
    ...
    ELSIF v_detalhe_len > 0 AND v_detalhe_sum > v_cartao + 0.05 THEN
      -- Cartão + boleto: o IAM lista primeiro as parcelas do cartão. Remove
      -- o prefixo cuja soma bate com o cartão e mantém só os boletos.
      v_acum := 0;
      v_corte := 0;
...
COMMENT ON FUNCTION public.iam_treinamento_financeiro(jsonb) IS
  'Financeiro IAM→GC. Cartão de crédito (contrato não pendente) é quitado à vista para a empresa, em qualquer nº de parcelas do cliente.';
