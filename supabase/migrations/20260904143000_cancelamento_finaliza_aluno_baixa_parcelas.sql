-- Ajuste do trigger de conclusão de cancelamento (04/09/2026, 2ª parte).
--
-- Caso EDUARDO HENRIQUE FLAUSINO: a ficha chegou na conclusão com
-- status_cancelamento = 'solicitado' (a gravação 'aguardando_conciliacao' da
-- formalização foi sobrescrita pelo loop de regravação do front). O trigger
-- marcou Cancelado mas, seguindo a regra do front (só baixa quando
-- 'aguardando_conciliacao'), deixou as 10 parcelas em aberto na ficha; a
-- gravação do front veio logo depois já "desatualizada" e a guarda de escrita
-- descartou as colunas financeiras. Resultado: aluno Cancelado com 10 parcelas
-- abertas e a multa paga (R$ 1.500) fora do card Pago.
--
-- Regra nova: aplica as parcelas revisadas do caso sempre que a ficha ainda
-- tiver parcela em aberto que não seja a multa — ou seja, sempre que a baixa
-- ainda não aconteceu, independente do sub-status.
...
BEGIN
  ...
  -- cancelada mas ficou com a baixa pendente). Ficha 'nenhum'/'revertido'
  -- fica como está — pode ser legado ou reversão posterior.
  IF COALESCE(v_stud.status_cancelamento, 'nenhum') NOT IN
     ('solicitado', 'aguardando_conciliacao', 'pagamento_multa_pendente', 'em_tratamento', 'juridico', 'cancelado') THEN
    RETURN;
  END IF;
...
-- Reparo: fichas já canceladas nas empresas ativas que ficaram com parcelas
-- em aberto apesar do caso finalizado (hoje: EDUARDO HENRIQUE FLAUSINO).
DO $$
DECLARE r record;
BEGIN
  FOR r IN
