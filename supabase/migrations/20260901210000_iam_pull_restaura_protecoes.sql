-- Restaura as proteções que a migração 20260828233000 removeu de
-- iam_control_upsert_one_contract, mantendo o merge de baixas que ela introduziu.
--
-- Contexto: o cron gc-iam-control-pull-incremental roda a cada 5 minutos e
-- reconstrói installments a partir do payload do IAM. Quando o payload não traz
-- o número de parcelas, iam_treinamento_financeiro cai no fallback de 1 parcela
-- (valor = sale_value - down_payment) e a marca como paga. Sem as proteções, o
-- cronograma real do GC (7 a 15 boletos) era substituído por essa parcela única
-- e as baixas registradas nas parcelas 2..N eram descartadas, porque o merge
-- casa por número de parcela.
--
-- Três proteções restauradas:
--   1. Contratos com status NOVO não são importados/atualizados.
--   2. Fichas cujo financeiro vem do Kamino não têm o financeiro sobrescrito.
--   3. Fichas que já possuem cronograma não têm o financeiro sobrescrito,
--      exceto quando o contrato está pendente (aí o IAM é a fonte).
--
-- Correção adicional: o merge passa a preservar paidValue e paidMarkedAt da
-- parcela baixada no GC, que antes eram perdidos em toda reescrita.
...
BEGIN
  ...
  IF v_student_id IS NOT NULL THEN
    ...
    -- exceto quando o contrato está pendente.
    SELECT coalesce(jsonb_array_length(s2.installments), 0) = 0
      INTO v_sem_parcelas
      FROM public.students s2
     WHERE s2.id = v_student_id;

    v_preserva_fin := v_kamino OR (NOT v_sem_parcelas AND NOT v_is_pendente);
