-- Pull do IAM Control: cada contrato (contrato_id) vira UMA ficha (09/09/2026).
--
-- Alinhamento com o IAM Control: a exportação gestao-contas/clientes passa a
-- incluir processos de migração/upgrade (ex.: PNL → Leader Skills). Com isso,
-- um mesmo aluno pode ter DOIS treinamentos com o mesmo id_treinamento na mesma
-- matrícula (duas vendas do mesmo produto). O casamento antigo — aluno IAM +
-- produto — colapsava as duas vendas numa única ficha (o segundo contrato
-- sobrescrevia o primeiro). Caso real: FABIANO ANDRÉ LEONELLO, Leader Skills,
-- contratos 1035 (R$ 2.000 PIX) e 1552 (R$ 2.561 cartão).
--
-- Regra nova em iam_control_upsert_one_contract():
--   1. Primeiro casa pela chave forte: students.iam_control_contrato_id =
--      contrato_id do treinamento (qualquer empresa — a ficha pode ter sido
--      arquivada).
--   2. Os casamentos de fallback (aluno IAM + produto, CPF + produto, identidade
--      e-mail/WhatsApp) só valem para ficha SEM contrato_id gravado ou com o
--      MESMO contrato_id. Ficha presa a outro contrato do mesmo produto não é
--      reaproveitada: o pull cria uma ficha nova.
--   3. Ficha nova de um 2º contrato do mesmo CPF + produto recebe
--      ciclo = 'Contrato IAM <id>' para respeitar students_unique_cpf_product_ciclo.
--   Sem contrato_id no payload, o comportamento é o anterior.
