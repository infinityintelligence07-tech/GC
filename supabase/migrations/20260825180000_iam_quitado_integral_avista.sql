...
BEGIN
  FOR v_i IN 0..greatest(jsonb_array_length(v_formas) - 1, -1) LOOP
    ...
    IF coalesce(v_fp->>'a_vista', '') IN ('true', '1', 'sim', 'TRUE')
    ...
    ELSIF public.iam_forma_is_parcelado(v_forma) THEN
      -- Cartão 1x no evento = à vista para o GC (não gera recebíveis futuros).
  ...
  -- CONCILIADO quitado: à vista no evento / valor_pago integral — não gera parcelas em aberto.
...
COMMENT ON FUNCTION public.iam_treinamento_financeiro(jsonb) IS
  'Converte treinamento IAM→GC. PENDENTE: separa entrada (PIX/link) do saldo parcelado. CONCILIADO quitado (à vista/valor_pago integral) não gera parcelas em aberto.';
