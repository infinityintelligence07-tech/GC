-- Mantém o sync IAM funcionando quando contratos já aprovados são arquivados.
-- Contratos que voltarem a exigir aprovação retornam automaticamente ao IAM - GC.
...
BEGIN
  ...
  -- O ID do IAM identifica o contrato mesmo quando ele está no arquivo.
  -- Isso evita recriar uma cópia no GC principal durante o próximo pull.
  IF v_iam_id IS NOT NULL THEN
    ...
    IF v_student_id IS NOT NULL THEN
      v_matched_by := 'iam_global_produto';
    END IF;
  END IF;
  ...
  IF v_student_id IS NOT NULL THEN
    UPDATE public.students s SET
      -- Pendência nova volta ao GC principal para gerar a fila IAM → GC.
      -- Status não pendente mantém a empresa atual, normalmente o arquivo.
      company_id                   = CASE
        WHEN v_status IN ('PENDENTE', 'PENDENTE_LINK', 'PENDENTE_PIX', 'PARA_CONCILIAR')
          THEN v_company_id
        WHEN v_status = 'CONCILIADO'
          ...
          AND NOT (
            ...
            OR (
              ...
              AND NOT EXISTS (
                SELECT 1
                FROM jsonb_array_elements(s.installments) inst
      ...
      -- Uma nova pendência precisa poder gerar uma nova fila no GC.
