-- Nome da ficha travado no GC (09/09/2026).
--
-- O pull do IAM Control reescreve o name da ficha a cada ciclo (~5 min). Quando
-- o cadastro no IAM está com o nome errado (ex.: contrato 1552 Leader Skills
-- cadastrado como "LEANDRO ROCHA" com CPF/e-mail do Fabiano André Leonello),
-- corrigir só no GC não adianta: o próximo pull desfaz.
--
-- Correções:
--   1. students.nome_manual_gc: quando true, o pull do IAM mantém o nome do GC
--      (demais dados continuam vindo do IAM).
--   2. iam_control_upsert_one_contract(): respeita a trava.
--   3. Reparo: ficha do contrato IAM 1552 renomeada para FABIANO ANDRÉ LEONELLO
--      e travada; item IAM > GC pendente segue o novo nome.

ALTER TABLE public.students
  ADD COLUMN IF NOT EXISTS nome_manual_gc boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.students.nome_manual_gc IS
  'Nome corrigido manualmente no GC: o pull do IAM Control não sobrescreve name.';
...
-- ─── Reparo: contrato IAM 1552 (Leader Skills) cadastrado no IAM como LEANDRO ROCHA ─
UPDATE public.students s SET
  name = 'FABIANO ANDRÉ LEONELLO',
  nome_manual_gc = true,
  history = coalesce(s.history, '[]'::jsonb) || jsonb_build_array(jsonb_build_object(
    'id', gen_random_uuid()::text,
    'date', now(),
    'text', 'Nome corrigido no GC: LEANDRO ROCHA → FABIANO ANDRÉ LEONELLO (contrato IAM 1552 está no IAM Control com o nome errado; CPF e e-mail são do Fabiano). Nome travado contra o pull do IAM.',
    'user', 'Sistema GC'
  ))
WHERE s.id = 'b424532f-f5f9-4bce-8c7a-a948cd9522d3'
  AND s.name = 'LEANDRO ROCHA';

UPDATE public.conciliacao_items ci SET
  student_name = 'FABIANO ANDRÉ LEONELLO'
WHERE ci.student_id = 'b424532f-f5f9-4bce-8c7a-a948cd9522d3'
  AND ci.student_name = 'LEANDRO ROCHA';
