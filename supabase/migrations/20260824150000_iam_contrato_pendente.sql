-- Contratos IAM Control PENDENTE (link/pix): metadados por ficha.

ALTER TABLE public.students
  ADD COLUMN IF NOT EXISTS iam_control_contrato_id text,
  ADD COLUMN IF NOT EXISTS iam_control_contrato_status text,
  ADD COLUMN IF NOT EXISTS iam_control_pendente_tipo text,
  ADD COLUMN IF NOT EXISTS iam_control_pendente_link text;

COMMENT ON COLUMN public.students.iam_control_contrato_id IS 'ID do contrato no IAM Control (turmas_alunos_treinamentos_contratos).';
COMMENT ON COLUMN public.students.iam_control_contrato_status IS 'Status de conciliação IAM: CONCILIADO, PENDENTE, etc.';
COMMENT ON COLUMN public.students.iam_control_pendente_tipo IS 'Quando PENDENTE: LINK ou PIX.';
COMMENT ON COLUMN public.students.iam_control_pendente_link IS 'URL de pagamento quando pendente_tipo = LINK.';
