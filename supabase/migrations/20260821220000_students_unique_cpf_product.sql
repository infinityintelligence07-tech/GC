-- Unicidade de aluno por contrato: CPF + treinamento (+ ciclo) por empresa.
-- Mesmo CPF em treinamentos diferentes continua permitido.

-- 1) Coluna normalizada de CPF (só dígitos)
ALTER TABLE public.students
  ADD COLUMN IF NOT EXISTS cpf_digits text
  GENERATED ALWAYS AS (regexp_replace(coalesce(cpf, ''), '[^0-9]', '', 'g')) STORED;

CREATE INDEX IF NOT EXISTS students_cpf_digits_idx
  ON public.students (company_id, cpf_digits)
  WHERE cpf_digits <> '';

-- 2) Resolve duplicatas atuais (mesmo company + cpf + product + ciclo):
--    mantém a "melhor" ficha e remove as demais.
WITH ranked AS (
  SELECT
    s.id,
    s.company_id,
    s.cpf_digits,
    lower(btrim(coalesce(s.product, ''))) AS product_key,
    lower(btrim(coalesce(s.ciclo, ''))) AS ciclo_key,
    row_number() OVER (
      PARTITION BY
        s.company_id,
        s.cpf_digits,
        lower(btrim(coalesce(s.product, ''))),
        lower(btrim(coalesce(s.ciclo, '')))
      ORDER BY
        (s.iam_control_aluno_id IS NOT NULL) DESC,
        coalesce(s.paid_installments, 0) DESC,
        coalesce(s.sale_value, 0) DESC,
        s.updated_at DESC NULLS LAST,
        s.created_at ASC NULLS LAST
    ) AS rn
  FROM public.students s
  WHERE coalesce(s.cpf_digits, '') <> ''
    AND length(s.cpf_digits) >= 11
),
losers AS (
  SELECT id FROM ranked WHERE rn > 1
)
DELETE FROM public.cancellation_cases cc
WHERE cc.student_id IN (SELECT id FROM losers);
...
CREATE UNIQUE INDEX IF NOT EXISTS students_unique_cpf_product_ciclo
  ON public.students (
    company_id,
    cpf_digits,
    lower(btrim(coalesce(product, ''))),
    lower(btrim(coalesce(ciclo, '')))
  )
  WHERE cpf_digits <> '' AND length(cpf_digits) >= 11;

COMMENT ON INDEX public.students_unique_cpf_product_ciclo IS
  'Um CPF por treinamento(+ciclo) na empresa. Mesmo CPF em produtos diferentes é permitido.';

-- 4) Upsert IAM: casa também por CPF + produto (contrato), além de id/telefone/email
...
begin
    ...
    -- 2) CPF + treinamento (mesmo contrato) — regra principal de unicidade
    ...
    -- 3) Soft-match telefone + e-mail + nome (só fichas sem vínculo IAM)
        ...
        from (
            ...
            from (
                ...
                where s.company_id = v_company_id
                  ...
                  -- Se o produto do payload veio preenchido, só casa no mesmo treinamento
                  and (
                    v_produto = ''
