-- Corrige sync IAM→GC: coluna usada pela RPC + company_id no INSERT (service_role sem auth.uid).

ALTER TABLE public.students
  ADD COLUMN IF NOT EXISTS iam_control_synced_at TIMESTAMPTZ;

COMMENT ON COLUMN public.students.iam_control_synced_at IS
  'Última sincronização recebida do IAM Control (webhook/pull).';
...
declare
    ...
    v_empatados    int     := 0;
    v_acao         text;
    -- Fallback: empresa IAM - GC (service_role não tem auth.uid / current_company_id)
...
begin
    ...
    -- Matricula mais recente: define produto e data de matricula.
    ...
    -- 2) Conferencia por telefone + email + nome.
    if v_student_id is null then
        ...
        from (
            ...
            from (
                ...
                from public.students s
                where s.iam_control_aluno_id is null
                  and (
