-- PENDENTE: não duplicar valor_entrada sobre parcelas_detalhe;
-- se valor_pago cobre a entrada, mantém down_payment pago.
...
BEGIN
  ...
  IF v_pendente AND v_down > 0.0049 THEN
    IF v_valor_pago >= v_down - 0.01 THEN
      -- Entrada paga → mantém down_payment.
      NULL;
    ELSE
  ...
  -- Entrada paga: se o detalhe já soma o contrato inteiro (inclui a entrada), remove a 1ª.
  IF v_down > 0.0049 AND v_entrada_restante < 0.01 AND v_sale > 0 THEN
    ...
    ELSIF v_sum_inst + v_down > v_sale + 0.05 AND abs(v_sum_inst - (v_sale - v_down)) > 0.05 THEN
      -- Recolapsa o aberto no restante do contrato.
  ...
  -- Entrada NÃO paga: prefixa só se não estourar o valor do contrato.
  IF v_entrada_restante > 0.0049 THEN
    IF abs(v_sum_inst - v_sale) < 0.05 THEN
      -- Agenda já cobre o contrato: não duplica; só marca a 1ª se for a entrada.
...
COMMENT ON FUNCTION public.iam_treinamento_financeiro(jsonb) IS
  'Converte treinamento IAM→GC. PENDENTE: entrada só vira parcela se não paga (valor_pago); nunca duplica sobre parcelas_detalhe.';
